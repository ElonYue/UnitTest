...
import com.cheng.unittest.dagger.ui.LoginDaggerPresenter;
...
public class LoginDaggerPresenterTest {
    ...
    public void setUp() {
        ...
        mPresenter = new LoginDaggerPresenter(MyApp.instance.githubApi);
    ...
    public void testGetIdentify() {
        ...
        // 验证方法被调用10次
        Mockito.verify(mvpView, Mockito.times(10)).countdownNext(anyString());
        // 时间到120秒
        rule.testScheduler.advanceTimeTo(120, TimeUnit.SECONDS);
        Mockito.verify(mvpView, Mockito.times(120)).countdownNext(anyString());
        // 验证倒计时完成方法被调用
